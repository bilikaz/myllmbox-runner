#!/usr/bin/env bash
# ple.sh — poke the demand-paged PLE table of the RUNNING serve (recipes/qwen38-flash-next) (no restart, no root).
#   ./ple.sh populate   pull the whole 26.8 GiB table into memory now (repeatable; each pass re-touches every page)
#   ./ple.sh mmap       eager steps (prefill/warm-ups) gather via the mmap/ATS path
#   ./ple.sh nvme       eager steps gather via O_DIRECT reads (no page cache growth)
#   ./ple.sh status     what the serve reports + table pages mapped / free / swap right now
# Mechanism: patch 05's watcher thread polls /cache/mbx-ple-{prewarm,mmap,nvme} every 2 s, acts, deletes the file,
# prints a "PLE MMAP:" line. /cache in the container = this lane's .data/cache on the host.
set -euo pipefail
NAME="${MBX_CONTAINER:-mbx-vllm}"
CACHE="$(cd "$(dirname "$0")" && pwd)/.data/cache"
mem() {

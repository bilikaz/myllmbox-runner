"""OpenAI-compatible image server for black-forest-labs/FLUX.2-klein-9B.

FLUX.2 Klein 9B: rectified-flow transformer + 8B Qwen3 text encoder, 4-step
distilled. Served via diffusers Flux2KleinPipeline (offline from /models).
...
OpenAI has no steps/guidance fields; we accept them as optional extensions
(clients pass via extra_body / extra form fields), defaulting to the distilled
4-step profile. response_format defaults to b64_json (we don't host URLs).
Bearer gating happens at the http2-proxy hop, not here.
"""
import base64
import io
import json
...
MODEL_PATH = os.environ.get("MODEL_PATH", "/models/black-forest-labs/FLUX.2-klein-9B")
MODEL_ID = os.environ.get("SERVED_MODEL_NAME", "black-forest-labs/FLUX.2-klein-9B")
# Per-model defaults via env: klein is step-distilled (4 steps); FLUX.2-dev is
# not (28-50 steps). Both use guidance ~4.
...
# Triton kernels JIT for the running arch (sm_121a on GB10). Loads BF16 then
# quantizes, so MODEL_PATH is the normal BF16 repo. (TORCHAO_NVFP4=1 kept as a
...
if os.environ.get("QUANT_PRELOADED"):
    ...
    # use_safetensors=False breaks the safetensors components ("no .bin in text_encoder").
    import torchao  # noqa: F401 — MUST import so the pickled torchao tensor subclasses register, else
    from torchao.prototype.mx_formats import inference_workflow  # noqa: F401  the FP4 weights won't deserialize
    ...
    # COMPILE_BLOCKS=0 disables the torch.compile pass. Compiling gives the
    # fastest steady-state, but then each new image SHAPE pays a one-time Triton
    # JIT stall (minutes). For varied-size / iterative work (e.g. comic panels),
    # set 0: no stalls ever, ~10-15% slower per image.
    if os.environ.get("COMPILE_BLOCKS", "1") != "0":
        try:
            pipe.transformer.compile_repeated_blocks(fullgraph=True)

#!/usr/bin/env python3
"""make-hibrid47.py — hibrid47 = hibrid46 with the PLE n-gram table re-quantized bf16 → NVFP4 (block 16), as a checkpoint.

hibrid46 shipped the table int3 (17.9 GiB, CPU-offload gather). hibrid47 ships it NVFP4 (28.6 GiB) so the serving image
can hold it RESIDENT ON THE GPU and gather in-forward (patch 03-ple-gpu-nvfp4.py) — +8-11 % engine steps and, on the
boss-animals gauntlet, 26/32 good vs ~16/32 for int3. Body tensors are hibrid46's, byte-identical (hardlinked).
...
runs on a serving box. Resumable per output shard (a .ok marker per file).
  ...
  docker run --rm --user $(id -u):$(id -g) -v /home/valdas/spark-vllm-docker/models:/models -v $PWD/builds/qwen38-flash-next/docker/make-hibrid47.py:/x.py:ro \\
      ...
      --dst /models/myllmbox/Qwen3.8-Flash-Next-hibrid47
"""

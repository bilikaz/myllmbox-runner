...
Activates when config.json declares `ple_quantization.format == "nvfp4"` (written by docker/make-hibrid47.py) AND
...
# B2. MBX_PLE_REPLICATE=1: every rank holds the FULL table (no row sharding, no all-reduce) — the A/B against sharded.
#     Costs a full copy per box (NVFP4 28.8 GB vs 14.4 sharded) → only with a smaller KV pin.
edits.append(("replicate knob",
    ...
    """        self.ngram_embedding = VocabParallelEmbedding(
            ...
            disable_tp=_mbx_nvfp4_gpu() and _mbx_ple_replicate(),   # MBX: full table per rank, no all-reduce
        )"""))
...
def _mbx_ple_replicate() -> bool:
    """MBX_PLE_REPLICATE=1 → every TP rank holds the whole table (disable_tp), gathers locally, no all-reduce."""
    import os as _os
    return _os.environ.get("MBX_PLE_REPLICATE", "0") == "1"
...
class _MbxNvfp4EmbeddingMethod(QuantizeMethodBase):
    ...
    def process_weights_after_loading(self, layer) -> None:
        ...
        # release whatever the load left in the allocators' caches (device + pinned host) — the table is in place now
        try:
            ...
            pass
        mode = "REPLICATED (full table, no all-reduce)" if _mbx_ple_replicate() else "row-sharded over TP"
        print(f"PLE: NVFP4 table resident on GPU ({exp} rows this rank, {mode}, global scale {float(layer.nvfp4_global):.4e})", flush=True)

    def apply(self, layer, x, bias=None):
        raise NotImplementedError("NVFP4 PLE table is gather-only")

    def embedding(self, layer, input_: torch.Tensor) -> torch.Tensor:
        # Contract = F.embedding: output keeps the INPUT's shape + [D] (VocabParallelEmbedding masks the result with
        # input_mask.unsqueeze(-1), i.e. [..., 1] against [..., D]). Gather on the flattened ids, reshape back.
        shape = input_.shape
        ...
        sc = layer.nvfp4_scales.index_select(0, ids).to(torch.float32) * layer.nvfp4_global   # [n, D/16]
...
def _mbx_nvfp4_load(self, name: str, loaded_weight: torch.Tensor) -> set[str]:
    ...
    if hi > lo:
        # BLOCKING copy on purpose: non_blocking from pageable (mmap) memory made PyTorch pin staging buffers that
        # stayed resident after the load (+6 GB host RSS on the worker, boot 2). Plain copy_ = small internal staging.
        param.data[lo - p0: hi - p0].copy_(loaded_weight[lo - start: hi - start])
        if kind == "packed":
